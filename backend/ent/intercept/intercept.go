...
// The AccountNodeHealthFunc type is an adapter to allow the use of ordinary function as a Querier.
type AccountNodeHealthFunc func(context.Context, *ent.AccountNodeHealthQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f AccountNodeHealthFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.AccountNodeHealthQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.AccountNodeHealthQuery", q)
}

// The TraverseAccountNodeHealth type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAccountNodeHealth func(context.Context, *ent.AccountNodeHealthQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAccountNodeHealth) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAccountNodeHealth) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AccountNodeHealthQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.AccountNodeHealthQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.AccountNodeHealthQuery:
		return &query[*ent.AccountNodeHealthQuery, predicate.AccountNodeHealth, accountnodehealth.OrderOption]{typ: ent.TypeAccountNodeHealth, tq: q}, nil
